...
def build_transform(cfg, mode, is_source):
    if mode=="train":
        ...
        if is_source:
            ...
            trans_list = [transform.GaussianBlur()] + trans_list
            trans_list += [transform.CutOut()] 
...
def build_dataset(cfg, mode='train', is_source=True, epochwise=False, rsc=None):
    assert mode in ['train', 'val', 'test', 'TTT', 'III', 'meta_val']
    # TTT: cityscape 训练集伪标签
    # III: cityscape 训练集真实标签
    # train: cityscape 训练集mixup的
    ...
    if mode=='train':
        ...
        print('cfg.DATASETS.TT.', cfg.DATASETS.TT.split('_')[-1])
        dataset = DatasetCatalog.get(cfg.DATASETS.TT, cfg.DATASETS.TT.split('_')[-1], num_classes=cfg.MODEL.NUM_CLASSES, max_iters=iters, transform=transform, cfg=cfg)
    elif mode=='III':
        print('cfg.DATASETS.II.', cfg.DATASETS.II.split('_')[-1])
        dataset = DatasetCatalog.get(cfg.DATASETS.II, cfg.DATASETS.II.split('_')[-1], num_classes=cfg.MODEL.NUM_CLASSES, max_iters=iters, transform=transform)
    elif mode=='meta_val':
        dataset = DatasetCatalog.get(cfg.DATASETS.TARGET_TRAIN, mode, num_classes=cfg.MODEL.NUM_CLASSES, max_iters=iters, transform=transform, rsc=rsc, cfg=cfg)
    return dataset
