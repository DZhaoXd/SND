...
class rand_mixer_v2():
    ...
    #       out_labels:  (w, h, 3)
        ...
        while True:
            ...
            if 'cityscapes' in self.cfg.DATASETS.TARGET_TRAIN:
            ...
            elif 'bdd' in self.cfg.DATASETS.TARGET_TRAIN:
            ...
            cdd_img = Image.open(cdd_img_path).convert('RGB')
            cdd_lbl = Image.open(cdd_label_path)
            cdd_lbl = cdd_lbl.resize(cdd_img.size)
            cdd_img = np.asarray(cdd_img, np.uint8)
            cdd_lbl = np.asarray(cdd_lbl, np.uint8)
        ...
        for i in range(forenum):
            ...
            #### rescale instance 
            randscale = 1.0 + np.random.rand() * 0.2
...
class cityscapesDataSetMeta(data.Dataset):
    def __init__(
        self,
        data_root,
        data_list,
        ...
        print('cityscapesDataSetMeta', data_list)
        print('cityscapesDataSetMeta, pseudo label:', pseudo)
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        # mixup for full image
        mix_up_flag=True
        if mix_up_flag:
            full_mix_image, full_mix_label = self.mixer.mix(np.asarray(image, dtype=np.uint8), self.mix_classes, self.mix_p)
            full_mix_label = np.asarray(label) * (full_mix_label==255) + full_mix_label * (full_mix_label!=255)
        else:
            full_mix_image = np.array(image)
            full_mix_label = np.array(label)
        full_mix_image = np.array(full_mix_image, dtype=np.float64) / 255
        full_mix_image -= [0.485, 0.456, 0.406]
        full_mix_image /= [0.229, 0.224, 0.225]
        full_mix_image = torch.from_numpy(full_mix_image.transpose(2, 0, 1)).float()

        return full_mix_image, torch.from_numpy(full_mix_label), name
