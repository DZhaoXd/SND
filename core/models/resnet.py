...
def _resnet(arch, block, layers, pretrained, progress, pretrained_weights, **kwargs):
    ...
    #pretrained_weights = 'pretrain/InfoMin_resnet101_e300.pth'
    #if pretrained:
    #    load_checkpoint(model, pretrained_weights, map_location='cpu')

    """
    load_path = "pretrain/InfoMin_resnet101_e300.pth"
    pretrain_dict = torch.load(load_path, map_location=torch.device('cpu'))['model']
    model_dict = {}
    for k, v in pretrain_dict.items():
        new_key = k.replace("module.encoder.", "")
        if 'head' not in new_key:
            model_dict[new_key] = v
    load_checkpoint(model, model_dict, map_location='cpu')
    """ 
